...
Context for working on the TypeSpec model tooling (`@phaselock/typespec-core` + the three
...
- `$onValidate` (`core/src/validate.ts`) runs `lowerProgram()` (store collisions, not-a-store,
  ...
  already has checker errors, and all core diagnostics are severity error, so emit never runs on a
